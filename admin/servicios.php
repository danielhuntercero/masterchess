            ...
            <?php if (count($servicios) > 0): ?>
                <?php foreach ($servicios as $servicio): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars(number_format($servicio['precio_hora'] ?? 0, 2, ',', '.')) . ' €'; ?></td>
                        <td><?php echo htmlspecialchars($servicio['profesor_nombre'] ?? 'N/A'); ?></td>
                        <td>
                            <a href="servicios_editar.php?id=<?php echo $servicio['id']; ?>" class="btn btn-sm btn-primary me-2"><i class="bi bi-pencil"></i> Editar</a>
                            <a href="servicios.php?action=delete&id=<?php echo $servicio['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Estás seguro de que quieres eliminar este servicio? Las inscripciones relacionadas no serán eliminadas.');"><i class="bi bi-trash"></i> Eliminar</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
